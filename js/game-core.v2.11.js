...
function renderCasino(){try{mccInitCasino()}catch(err){console.error('Meme casino render error',err)}const cash=$('#casinoCashReadout');if(cash)cash.textContent=fmt(state.cash)}
  ...
  /* ==========================================================
     ...
     ========================================================== */
  const PF_UPDATE_ID='v35-campaign-specialty-admin-jet-20260913';
  const PF_VERSION='3.5';
  const PF_GLITCH_PACK_CHANCE=1/50;
  const JET_LUMAGUI_CARD_CHANCE=1/1000;
  const JET_LUMAGUI_CARD={id:'packforge_jet_lumagui',name:'Jet Lumagui',rarity:'Jet Lumagui',theme:'medieval',value:420,power:1,jet:true,flavor:'A quiet figure in the haze. Smoke curls across the print, but the card itself never explains why.'};
  ...
  sellValue=function(card,level,copy){if(card?.crue)return .01;if(card?.jet)return 420;return pfBaseSellValue(card,level,copy)};
  ...
  function pfRenderChances(){
    ...
    add('JET LUMAGUI TIER','Jet Lumagui',pfPackProb(JET_LUMAGUI_CARD_CHANCE),'packs','Own one-card rarity: exactly 1 in 1,000 card rolls (about 1 in 200.4 normal five-card packs). Power 1 · exact Sell $420 · smoky card treatment.');
  ...
  const pfV35InspectBase=showInspect;showInspect=function(id,level=1,sig){pfV35InspectBase(id,level,sig);const c=cardMap[id];if(c?.jet){const theme=$('#inspectModal .inspect-theme');if(theme)theme.textContent='1 in 1,000 cards · Jet Lumagui Rarity · Power 1 · Sell $420'}};
