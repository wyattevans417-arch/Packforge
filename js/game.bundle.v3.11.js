(() => {
  ...
  const GHOST_CARD={id:'packforge_unlisted_null',name:'The Unlisted Print',rarity:'Ghost',theme:'medieval',value:500000,power:350000,secret:true,ghost:true,flavor:'No set number. No printing record. No explanation.'};
  ...
  for(const it of Object.values(state.inventory||{}))for(const ld of Object.values(it?.levels||{}))for(const cp of (ld?.copies||[])){
    if(cp.variant==='Glitch'||cp.variant==='Glitched')cp.variant='Shattered';if(cp.variant==='Negative')cp.variant='Prismatic';if(cp.mutation==='goldwave')cp.mutation='holosurge';
  ...
  function createPackPulls(t,mutation,autoMode=false){
    ...
    let packMode=null;
    const forcedMode=!tutorialTrainingMode&&PF_PACK_MODE_DEFS[af.godMode]?af.godMode:null;
    if(forcedMode){packMode=forcedMode;af.godMode=null}
...
function recordGamble(name,wager,returned){pfCasinoFloat(returned-wager);const g=gameStats(name);g.plays++;g.wagered+=wager;g.returned+=returned;if(returned>wager){g.wins++;g.biggestWin=Math.max(g.biggestWin||0,returned-wager);state.casino.wins++;state.casino.biggestWin=Math.max(state.casino.biggestWin||0,returned-wager)}else if(returned<wager){g.losses++;state.casino.losses++}save()}
...
(function(){
  ...
  function minesRevealAll(){document.querySelectorAll('#minesGrid .mines-tile').forEach(tile=>{const i=+tile.dataset.idx;if(minesSet.has(i)){tile.classList.add('revealed-mine');tile.textContent='💣'}tile.classList.add('disabled');tile.disabled=true})}
  function minesBuildGrid(active=false){const grid=$m('minesGrid');if(!grid)return;grid.innerHTML='';for(let i=0;i<25;i++){const tile=document.createElement('button');tile.type='button';tile.className='mines-tile'+(active?'':' disabled');tile.dataset.idx=i;tile.setAttribute('aria-label',active?`Mines tile ${i+1}`:`Mines tile ${i+1} — start a round first`);grid.appendChild(tile)}}
  function minesEnd(){minesActive=false;$m('minesCashoutBtn').disabled=true;$m('minesStartBtn').disabled=false}
  function minesStart(){if(minesActive)return;const bet=parseBet($m('minesBetInput')?.value),count=parseInt($m('minesCountSelect')?.value||'1',10);if(!(bet>=0))return alertMsg('Invalid Bet','Enter a valid bet of $0.00 or more.');if(!(count>=1&&count<=24))return alertMsg('Invalid Mines','Choose 1 to 24 mines.');if(bet>state.cash)return alertMsg('Not enough cash',`You only have ${fmt(state.cash)}.`);if(!spendCash(bet))return;minesActive=true;minesBet=bet;minesCount=count;minesSet=new Set();minesRevealed=new Set();minesSafe=0;minesMult=1;while(minesSet.size<count)minesSet.add(Math.floor(Math.random()*25));minesBuildGrid(true);$m('minesCashoutBtn').disabled=true;$m('minesStartBtn').disabled=true;$m('minesStatus').textContent=`Game started! Mines: ${count}. Pick tiles...`;minesUpdate()}
  function minesPick(i,tile){if(!minesActive||minesRevealed.has(i))return;minesRevealed.add(i);if(minesSet.has(i)){tile.classList.add('revealed-mine','disabled');tile.disabled=true;tile.textContent='💣';$m('minesStatus').textContent=`BOOM! You hit a mine and lost ${fmt(minesBet)}.`;minesRevealAll();recordGamble('mines',minesBet,0);minesEnd();sync();return}minesSafe++;tile.classList.add('revealed-safe','disabled');tile.disabled=true;tile.textContent='💎';minesMult=minesMultiplier(minesSafe,minesCount);minesUpdate();$m('minesCashoutBtn').disabled=false;$m('minesStatus').textContent=`Safe picks: ${minesSafe} • Mines: ${minesCount} • Cash out anytime.`;try{achMeta().maxMinesSafe=Math.max(achMeta().maxMinesSafe||0,minesSafe);checkAchievements(true)}catch(e){}}
  ...
  function bindGames(){
    document.querySelectorAll('[data-coin-choice]').forEach(b=>b.addEventListener('click',()=>{if(coinSpinning)return;coinChoice=b.dataset.coinChoice||'Heads';document.querySelectorAll('[data-coin-choice]').forEach(x=>x.classList.toggle('active',x===b))}));$m('coinFlipBtn')?.addEventListener('click',coinFlip);$m('minesStartBtn')?.addEventListener('click',minesStart);$m('minesCashoutBtn')?.addEventListener('click',minesCashout);const minesGrid=$m('minesGrid');if(minesGrid&&!minesGrid.dataset.bound){minesGrid.dataset.bound='1';minesGrid.addEventListener('click',e=>{const tile=e.target.closest?.('.mines-tile');if(!tile||!minesActive||tile.classList.contains('disabled'))return;minesPick(+tile.dataset.idx,tile)});minesBuildGrid(false)}$m('plinkoDropBtn')?.addEventListener('click',plinkoDrop);$m('slotsSpinBtn')?.addEventListener('click',()=>slotsSpin(false));$m('slotsAutoBtn')?.addEventListener('click',slotsAuto);$m('slotsStopBtn')?.addEventListener('click',slotsStopAuto);document.querySelectorAll('#mcc-slots [data-slot-bet]').forEach(b=>b.addEventListener('click',()=>{const el=$m('slotsBetInput');if(el){el.value=b.dataset.slotBet;slotsLastBet=Number(b.dataset.slotBet)||slotsLastBet;if($m('slotsBetReadout'))$m('slotsBetReadout').textContent=fmt(slotsLastBet)}}));$m('slotsBetInput')?.addEventListener('input',()=>{const n=parseBet($m('slotsBetInput').value);if(n>=0){slotsLastBet=n;if($m('slotsBetReadout'))$m('slotsBetReadout').textContent=fmt(n)}});$m('bjDealBtn')?.addEventListener('click',bjDeal);$m('bjHitBtn')?.addEventListener('click',bjHit);$m('bjStandBtn')?.addEventListener('click',bjStand);$m('bjDoubleBtn')?.addEventListener('click',bjDouble);$m('rouletteSpinBtn')?.addEventListener('click',rouletteSpin);$m('rouletteClearBtn')?.addEventListener('click',rouletteClear);$m('rouletteTable')?.addEventListener('click',e=>{const cell=e.target.closest('[data-bet]');if(cell)roulettePlace(cell.dataset.bet)});document.addEventListener('keydown',e=>{if(currentTab!=='slots'||e.code!=='Space'||e.repeat||/INPUT|TEXTAREA|SELECT|BUTTON/.test(document.activeElement?.tagName||''))return;e.preventDefault();slotsSpin(false)});rouletteEnsureTable();rouletteDrawWheel(rouletteWheelRotation);plinkoDraw();plinkoStatsUI();slotsRender(slotsMatrix);slotsUpdateControls();bjRender();
  }
  window.__pfMinesDebug={multiplier:minesMultiplier,build:minesBuildGrid,start:minesStart,pick:minesPick,cashout:minesCashout,get active(){return minesActive},get safe(){return minesSafe},get mineCount(){return minesCount}};
})();
function buildCasino(){
  ...
  if(cf&&!cf.dataset.pfModernCoin)cf.dataset.pfModernCoin='1';if(pd)pd.onclick=dropPlinko;if(cs)cs.onclick=startCrash;if(cc)cc.onclick=cashoutCrash;if(ss)ss.onclick=spinSlots;if(mm)mm.onclick=spinMysteryMachine;
...
function renderCasino(){try{mccInitCasino()}catch(err){console.error('Meme casino render error',err)}const cash=$('#casinoCashReadout');if(cash)cash.textContent=fmt(state.cash)}
  ...
  const ACHIEVEMENTS=[
    ...
    {id:'variant_spectrum',icon:'🌈',name:'Variant Spectrum',desc:'Have discovered every non-Normal variant type.',cat:'Variants',tier:'Secret',check:s=>['Foil','Holo','Gold','Prismatic','Serialized','Shattered'].every(v=>!!s.meta.variantsEver[v])},
    ...
    {id:'jet_first',icon:'🌫️',name:'Smoke Signal',desc:'Pull Jet Lumagui.',cat:'Rare Pulls',tier:'Hard',check:s=>(state.quirkStats?.jetLumagui||0)>=1},
    {id:'jet_five',icon:'🔥',name:'Five Jets',desc:'Pull Jet Lumagui five times.',cat:'Rare Pulls',tier:'Brutal',check:s=>(state.quirkStats?.jetLumagui||0)>=5},
    {id:'shattered_five',icon:'🪟',name:'Broken Display Case',desc:'Own five Shattered cards at the same time.',cat:'Variants',tier:'Hard',check:s=>s.copies.filter(x=>(x.copy?.variant||'Normal')==='Shattered').length>=5},
    {id:'casino_regular',icon:'♣',name:'Casino Regular',desc:'Play 50 total casino rounds.',cat:'Casino',tier:'Advanced',check:s=>Object.values(state.stats.games||{}).reduce((n,g)=>n+(Number(g?.plays)||0),0)>=50},
    {id:'mines_five',icon:'💣',name:'Careful Steps',desc:'Reveal five safe Mines tiles in one run.',cat:'Casino',tier:'Advanced',check:s=>(s.meta.maxMinesSafe||0)>=5}
  ];
  ...
  setInterval(()=>{if(document.hidden)return;const now=Date.now(),dt=Math.min(4,Math.max(0,(now-showcaseIncomeClock)/1000));showcaseIncomeClock=now;const rate=showcaseBonuses().passivePerSec;if(rate>0&&dt>0){const gain=rate*dt;earn(gain);state.showcase.incomeEarned=(state.showcase.incomeEarned||0)+gain;state.showcase.lastIncomeAt=now;renderHUD();if(!$('#view-collection').classList.contains('hidden')&&currentBagTab==='showcase'){const el=$('.showcase-income b');if(el)el.textContent=`${showcaseRateFmt(rate)}/sec`}}},2000);
  /* Potions are retired; no background potion timer is scheduled. */
  ...
  /* ==========================================================
     ...
     ========================================================== */
  const PF_UPDATE_ID='v311-chase-stability-20260913';
  const PF_VERSION='3.11';
  ...
  function pfRenderChances(){
    ...
    add('STANDARD VARIANTS','Shattered',variantDefs.Shattered?.chance||1/2000,'packs','Rigid fractured-glass finish.');
  ...
  function pfRenderJournal(){const g=$('#pfJournalGrid');if(!g)return;const hasSecret=allCards.some(c=>c.secret&&state.history?.[c.id]?.totalPulled>0),q=state.quirkStats||{},journalEntries=[['🌟','God Pack',q.godPacks||0,'A wrapper where every card broke the normal rules.'],['🌫️','Jet Lumagui',q.jetLumagui||0,`${pfOwnedCount(JET_LUMAGUI_CARD.id)} currently owned · exact natural odds 1 in 1,000 cards.`],['👻','Ghost / Unlisted',q.ghostCards||0,`${pfOwnedCount(GHOST_CARD.id)} currently owned.`],['📦','Factory Pack Anomaly',q.packAnomalies||0,'A four- or six-card wrapper.'],['⚠','Factory Error Card',q.factoryErrors||0,'A true production defect with collector consequences.'],['✦','Hidden Holo Pattern',q.holoPatterns||0,'Swirl, Starburst, Galaxy, or Double Holo.'],['🏷','Perfect Black Label',q.blackLabels||0,'The rarest normal grading result.'],['👁','Secret Card',hasSecret?1:0,'A card hidden outside the normal 100-card checklist.'],['♜','Crue Lowe',q.crueLowe||0,`${pfOwnedCount(PF_CRUE_LOWE_CARD.id)} currently owned.`]];g.innerHTML=journalEntries.map(([icon,name,count,desc])=>{const seen=count>0;return `<article class="pf-journal-entry ${seen?'seen':'locked'}"><div class="pf-journal-icon">${seen?icon:'?'}</div><div><small>${seen?'DISCOVERED':'UNDISCOVERED'}</small><b>${seen?name:'UNDISCOVERED'}</b><span>${seen?desc:'Keep playing. PackForge does not reveal what is missing.'}</span></div>${seen?`<strong>×${Number(count).toLocaleString()}</strong>`:''}</article>`}).join('')}
  ...
  function pfOriginalAchievementCount(){const u=ensureAchievementState().unlocked||{};return Object.keys(u).filter(id=>u[id]&&PF_V33_ORIGINAL_ACHIEVEMENT_IDS.has(id)).length}
  const PF_V33_ACHIEVEMENTS=[
    ...
    {id:'v33_glitch',icon:'🪟',name:'First Fracture',desc:'Own a Shattered variant.',cat:'Rare Pulls',tier:'Hard',check:s=>s.variants.has('Shattered')},
  ...
  const PF_V34_ACHIEVEMENTS=[
    ...
    {id:'v34_glitch_10',icon:'🪟',name:'Glass Cabinet',desc:'Own 10 Shattered cards.',cat:'Variants',tier:'Hard',check:s=>s.copies.filter(x=>(x.copy?.variant||'Normal')==='Shattered').length>=10},
  ...
  const PF_V311_ACHIEVEMENTS=[
    {id:'v311_mines_15',icon:'💣',name:'Minefield Cartographer',desc:'Reveal 15 safe Mines tiles in one run.',cat:'Casino',tier:'Brutal',check:s=>(s.meta.maxMinesSafe||0)>=15},
    {id:'v311_ultra_5',icon:'✦',name:'Ultra Constellation',desc:'Own five Ultra cards at the same time.',cat:'Collection',tier:'Hard',check:s=>s.copies.filter(x=>x.card.rarity==='Ultra').length>=5},
    {id:'v311_jet_10',icon:'🌫️',name:'Jet Stream',desc:'Pull Jet Lumagui ten times.',cat:'Rare Pulls',tier:'Brutal',check:s=>(state.quirkStats?.jetLumagui||0)>=10},
    {id:'v311_shattered_25',icon:'🪟',name:'Wall of Glass',desc:'Own 25 Shattered cards at the same time.',cat:'Variants',tier:'Brutal',check:s=>s.copies.filter(x=>(x.copy?.variant||'Normal')==='Shattered').length>=25}
  ];
  if(!ACHIEVEMENTS.some(a=>a.id==='v311_mines_15'))ACHIEVEMENTS.push(...PF_V311_ACHIEVEMENTS);
  ...
  const PF_V37_VARIANTS=['Foil','Holo','Gold','Prismatic','Shattered','Serialized'];
  function pfV37VariantChance(v){
    if(v==='Shattered')return variantDefs.Shattered?.chance||0;
  ...
  /* ==================== PackForge v3.10 — cleanup + chase-card polish ==================== */
  ...
  if(typeof PF_V37_VARIANTS!=='undefined'){PF_V37_VARIANTS.splice(0,PF_V37_VARIANTS.length,...['Foil','Holo','Gold','Prismatic','Shattered','Serialized']);}
  ...
  function pfV310MasterVariant(i,total){if(i===total-1){const r=Math.random();if(r<.05)return'Serialized';if(r<.25)return'Shattered';return'Prismatic'}const r=Math.random();if(r<.53)return'Holo';if(r<.77)return'Gold';if(r<.95)return'Prismatic';if(r<.99)return'Shattered';return'Serialized'}
  ...
  for(const [cid,it] of Object.entries(state.inventory||{}))for(const ld of Object.values(it.levels||{}))for(const cp of ld.copies||[]){if(cp.variant==='Negative')cp.variant='Prismatic';if(cp.variant==='Glitched'||cp.variant==='Glitch'){cp.variant='Shattered';cp.glitchTarget=null}if(cp.mutation==='goldwave')cp.mutation='holosurge'}
  ...
  const pfV310HealthBase=pfV35HealthCheck;pfV35HealthCheck=function(){const h=pfV310HealthBase();h.checks.push(['Shattered variant',variantDefs.Shattered?.chance===1/2000],['Retired variants removed',!variantDefs.Negative&&!variantDefs.Glitched],['Golden Wave removed',!packMutations.goldwave],['Ghost power/value ratio',effectiveCardPower(GHOST_CARD,1,{variant:'Normal'})===350000&&sellValue(GHOST_CARD,1,{variant:'Normal'})===5000000]);h.failed=h.checks.filter(x=>!x[1]);h.ok=h.failed.length===0;return h};
  ...
  pfShowWhatsNew=function(){pfClearUpdateBadge();const m=document.getElementById('confirmModal');m.classList.remove('hidden');m.innerHTML=`<div class="modal pf-whats-new pf-whats-new-v33 fade-in"><div class="modal-head"><h3>PackForge v${PF_VERSION}</h3><button class="close-x" id="pfWhatsNewClose">×</button></div><div class="modal-body"><div class="pf-update-kicker">CLEANUP & CARD POLISH</div><h2>Big changes</h2><div class="pf-change-grid"><span>👻 Ghost / Unlisted rebuilt as the 1-in-1,000,000 animated chase card; Power is now scaled like a Secret-tier chase card instead of matching dollars 1:1</span><span>🪟 Shattered uses rigid pulsing broken-glass cracks; Gold gets a premium molten finish</span><span>🎰 Casino adds Heads/Tails choice and floating win/loss feedback</span><span>⚔ Rare 50,000-Power Campaign can appear with premium rewards</span><span>⚙ Settings simplified to Audio, Video, Codes, Reset, and Admin with three-step reset verification</span></div><div class="pf-update-divider"></div><h2>Major upgrades since the original build</h2><div class="pf-change-grid cumulative"><span>⚡ Split/PWA architecture and Chromebook-focused performance work</span><span>🌟 Semi-God / God / Archive Pack chase system and Campaign-only specialty rewards</span><span>🛡 Expanded Admin tools, Extra stats/chances/achievements, and Physics reference</span><span>👁 Secret, Ghost, Jet Lumagui, Crue Lowe, Shattered, and upgraded premium card finishes</span></div><button class="grade-action" id="pfWhatsNewDone">Got it</button></div></div>`;const close=()=>{m.classList.add('hidden');m.innerHTML=''};document.getElementById('pfWhatsNewClose').onclick=close;document.getElementById('pfWhatsNewDone').onclick=close};

  /* ---------- PackForge v3.11 — chase FX + stability ---------- */
  const PF_GHOST_BASE_POWER=350000;
  const pfV311HealthBase=pfV35HealthCheck;
  pfV35HealthCheck=function(){
    const h=pfV311HealthBase();
    const normalGhost={variant:'Normal',mutation:null,serial:null,grade:null};
    h.checks.push(
      ['Ghost natural odds',GHOST_CARD_CHANCE===1/1000000],
      ['Ghost power scaled',effectiveCardPower(GHOST_CARD,1,normalGhost)===PF_GHOST_BASE_POWER],
      ['Ghost sell remains chase-tier',sellValue(GHOST_CARD,1,normalGhost)===5000000],
      ['Mines debug bridge',!!window.__pfMinesDebug&&typeof window.__pfMinesDebug.multiplier==='function'],
      ['Mines multiplier finite',!!window.__pfMinesDebug&&Number.isFinite(window.__pfMinesDebug.multiplier(1,3))&&window.__pfMinesDebug.multiplier(1,3)>1],
      ['Ultra card tier present',rarityOrder.includes('Ultra')]
    );
    h.failed=h.checks.filter(x=>!x[1]);h.ok=h.failed.length===0;return h
  };
  pfShowWhatsNew=function(){pfClearUpdateBadge();const m=document.getElementById('confirmModal');m.classList.remove('hidden');m.innerHTML=`<div class="modal pf-whats-new pf-whats-new-v33 fade-in"><div class="modal-head"><h3>PackForge v${PF_VERSION}</h3><button class="close-x" id="pfWhatsNewClose">×</button></div><div class="modal-body"><div class="pf-update-kicker">CHASE FX + STABILITY</div><h2>Big changes</h2><div class="pf-change-grid"><span>👻 The Unlisted Print now has a full moving spectral field and Secret-level animated presence without copying Secret's motion</span><span>✦ Ultra cards use drifting tiny star-shaped lights instead of ordinary dots</span><span>⚔ Ghost Power is scaled like a chase-card Power/value relationship instead of matching its $5M sell price 1:1</span><span>💣 Mines now builds a visible 25-tile board immediately and uses a hardened delegated input path</span><span>⚡ Deployment assets are consolidated and repeat loads are cache-first to cut Vercel requests</span></div><button class="grade-action" id="pfWhatsNewDone">Got it</button></div></div>`;const close=()=>{m.classList.add('hidden');m.innerHTML=''};document.getElementById('pfWhatsNewClose').onclick=close;document.getElementById('pfWhatsNewDone').onclick=close};
...
/* PackForge PWA: throttle service-worker update checks so repeat opens do not create needless edge requests. */
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>{
    try{
      const KEY='pf_sw_check_v311', now=Date.now(), last=Number(localStorage.getItem(KEY)||0), due=!navigator.serviceWorker.controller||now-last>24*60*60*1000;
      if(!due)return;
      navigator.serviceWorker.register('./service-worker.js',{updateViaCache:'all'}).then(()=>{try{localStorage.setItem(KEY,String(now))}catch(e){}}).catch(()=>{});
    }catch(e){navigator.serviceWorker.register('./service-worker.js',{updateViaCache:'all'}).catch(()=>{})}
  },{once:true});
}
