(() => {
  ...
  const ARCHIVE_GOD_PACK_CHANCE=1/1000000000; // Impossible Archive: one of every card in the selected set.
  ...
  const PF_PACK_EVENT_DEFS={
    ...
    archive:{tier:'god',name:'Archive God Pack',short:'ARCHIVE GOD PACK',emoji:'∞',desc:'One copy of every card in the selected set. Intentionally almost impossible.',chance:GOD_PACK_MODE_CHANCES.archive}
  };
  ...
  const rewardCodeDefs={
    ...
    "GoD-ArChIvE":{label:'Queue 1 impossible Archive God Pack + grant a core test pack',apply:()=>{state.adminForces.godMode='archive';grantRandomCorePacks(1)},rewardId:"PF32-GOD-ARCHIVE"}
  };
  ...
  function renderShop(){
    ...
    $$('[data-shop-buy]').forEach(b=>b.onclick=()=>buyShopPack(b.dataset.shopBuy));renderPremiumPackShop();
  }
  ...
  function weightedCard(theme,mutation,autoMode=false,qualityBoost=1){
    if(!tutorialTrainingMode&&Math.random()<GHOST_CARD_CHANCE*(autoMode?.75:1)*overdriveBoost()){state.quirkStats.ghostCards=(state.quirkStats.ghostCards||0)+1;return {card:GHOST_CARD,nearMiss:null}}
  ...
  function pfCardRollChance(card,theme,auto=false){
    const scale=auto?.75:1;if(card.ghost)return GHOST_CARD_CHANCE*scale;if(card.secret)return (1-GHOST_CARD_CHANCE*scale)*(SECRET_CHANCE*scale)/Math.max(1,theme.secretCards.length);
    const table=auto?AUTO_RARITY_ODDS:RARITY_ODDS,count=theme.cards.filter(c=>c.rarity===card.rarity).length;return (1-GHOST_CARD_CHANCE*scale)*(1-SECRET_CHANCE*scale)*(table[card.rarity]||0)/Math.max(1,count)
  }
  function pfOddsCardEntries(theme){return [...theme.allCards,GHOST_CARD].map(card=>({card,manual:pfCardRollChance(card,theme,false),auto:pfCardRollChance(card,theme,true)}))}
...
function renderCasino(){try{mccInitCasino()}catch(err){console.error('Meme casino render error',err)}const cash=$('#casinoCashReadout');if(cash)cash.textContent=fmt(state.cash)}
  ...
  /* ==========================================================
     ...
     ========================================================== */
  const PF_UPDATE_ID='odds-godpacks-v32-20260913';
  const PF_VERSION='3.2';
  ...
  (()=>{
    ...
    pfCollectCampaignSlot=function(slot){
      ...
      const packRewards=[];const packCount=(def.packs||0)+tier.bonusPacks;for(let i=0;i<packCount;i++){const p=campaignAddPack(null,false);if(p)packRewards.push(p)}const extraChance=Math.min(.98,(def.packChance||0)+(tier.extraPackChance||0)+teamBonus*.8);if(Math.random()<extraChance){const p=campaignAddPack(null,false);if(p)packRewards.push(p)}let specialWon=false;const specialChance=Math.min(.90,(def.special||0)+(tier.specialBonus||0)+teamBonus*.18);if(Math.random()<specialChance){const p=campaignAddPack(null,true);if(p){packRewards.push(p);specialWon=true}}let potionWon=null;const potionChance=Math.min(.95,(def.potionChance??fallbackRewards.potionChance??0)+(tier.name==='Master'?.05:tier.name==='Gold'?.025:0)+teamBonus*.25);if(Math.random()<potionChance){const ids=Object.keys(potionDefs),id=ids[Math.floor(Math.random()*ids.length)];addPotion(id,1);potionWon=potionDefs[id]}const hiddenSetWon=tryUnlockCampaignSet();state.campaigns.completed.total=(state.campaigns.completed.total||0)+1;state.campaigns[key]=null;state.stats.campaignsCompleted=(state.stats.campaignsCompleted||0)+1;state.stats.campaignCash=(state.stats.campaignCash||0)+cash;state.stats.campaignPacksWon=(state.stats.campaignPacksWon||0)+packRewards.filter(p=>!p.special).length;state.stats.campaignSpecialsWon=(state.stats.campaignSpecialsWon||0)+(specialWon?1:0);state.stats.campaignPotionsWon=(state.stats.campaignPotionsWon||0)+(potionWon?1:0);const summary={name:def.name,tier:tier.name,power:a.power,cash,packs:packRewards,specialWon,potionWon,hiddenSetWon,speed:a.speed||campaignSpeed(def,a.power)};generateCampaignOffers();sfx('grade');confetti(tier.name==='Master'?28:14);save();renderAll();renderCampaigns();showCampaignRewardPopup(summary)
    };
    ...
    function pfRequestEligible(){return allOwnedCopies().filter(x=>!x.card.secret&&!x.card.ghost&&!x.card.crue&&!x.copy.locked&&!x.copy.gradingUntil&&!copyIsCampaignCommitted(x.copy.uid)&&!copyIsShowcased(x.copy.uid))}
    ...
    renderCampaigns=function(){
      ...
      let req=$q('#pfCollectorRequestMount');if(!req){req=document.createElement('div');req.id='pfCollectorRequestMount';req.className='pf-request-mount';view.insertAdjacentElement('afterend',req)}req.innerHTML=pfRequestHTML();$$q('[data-pf-request]').forEach(b=>b.onclick=()=>pfFulfillRequest(b.dataset.pfRequest));
      $$q('.campaign-choice-card[data-campaign-choice]').forEach(card=>{const def=state.campaigns.offers.find(o=>o.offerId===card.dataset.campaignChoice);if(!def||card.querySelector('.pf-offer-focus'))return;const f=pfFocus(def),tag=document.createElement('div');tag.className='pf-offer-focus';tag.innerHTML=`<b>${f.label}</b><span>${f.detail}</span>`;card.querySelector('h3')?.insertAdjacentElement('afterend',tag)});
    ...
    window.pfCampaignHeartbeat=function(){
      ...
      const clock=$q('[data-pf-request-clock]');if(clock){const st=pfRequestState(),left=Math.max(0,Number(st.expiresAt||0)-now),m=Math.floor(left/60000),sec=Math.floor((left%60000)/1000);clock.textContent=`Refresh ${m}:${String(sec).padStart(2,'0')}`;if(left<=0)renderCampaigns()}
    };
  
    /* ---------- Inspect fullscreen + zoom ---------- */
    const pfInspectBase=showInspect;
    showInspect=function(...args){
      pfInspectBase(...args);const modal=$q('#inspectModal'),scene=modal?.querySelector('.inspect-scene'),rot=modal?.querySelector('.inspect-rotator');if(!modal||!scene||!rot)return;
      modal.classList.remove('pf-inspect-fullscreen');rot.style.setProperty('--pf-card-zoom','1');
      if(!scene.querySelector('.pf-inspect-view-tools')){const tools=document.createElement('div');tools.className='pf-inspect-view-tools';tools.innerHTML=`<button type="button" data-pf-fullscreen title="Fullscreen card" aria-label="Fullscreen card">⛶</button><button type="button" data-pf-zoom-out aria-label="Zoom out">−</button><input type="range" min="70" max="180" value="100" step="5" aria-label="Card zoom"><button type="button" data-pf-zoom-in aria-label="Zoom in">＋</button><span>100%</span>`;scene.appendChild(tools);const slider=tools.querySelector('input'),read=tools.querySelector('span'),apply=()=>{const n=clamp(Number(slider.value)||100,70,180);slider.value=n;read.textContent=n+'%';rot.style.setProperty('--pf-card-zoom',String(n/100))};tools.querySelector('[data-pf-fullscreen]').onclick=()=>{modal.classList.toggle('pf-inspect-fullscreen');tools.querySelector('[data-pf-fullscreen]').textContent=modal.classList.contains('pf-inspect-fullscreen')?'⤢':'⛶'};tools.querySelector('[data-pf-zoom-out]').onclick=()=>{slider.value=Math.max(70,Number(slider.value)-10);apply()};tools.querySelector('[data-pf-zoom-in]').onclick=()=>{slider.value=Math.min(180,Number(slider.value)+10);apply()};slider.oninput=apply}
    };
