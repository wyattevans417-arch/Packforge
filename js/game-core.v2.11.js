(() => {
  ...
  const AUTO_RARITY_ODDS={Common:.73,Uncommon:.22,Rare:.035,Epic:.0105,Legendary:.00325,Mythic:.0009,Divine:.000275,Ultra:.000075}; // Auto Open halves every Rare+ base chance; removed probability goes to Common.
  ...
  const premiumPackDefs={
    variant:{id:'variant',name:'Variant Pack',emoji:'🌈',cost:300,sell:180,gradient:'linear-gradient(135deg,#4f7cff,#c55cff,#ff6ea8)',glow:'rgba(177,103,255,.24)',desc:'Built for variant hunting.',effect:'Exactly 2 cards receive a standard variant. Premium variants are possible.'},
    highroller:{id:'highroller',name:'High-Roller Pack',emoji:'🎲',cost:550,sell:330,gradient:'linear-gradient(135deg,#8f571f,#f0b94c,#6d3d18)',glow:'rgba(255,190,77,.22)',desc:'Expensive rarity chase.',effect:'No Common or Uncommon cards. Every card is Rare or better.'},
    vintage:{id:'vintage',name:'Vintage Pack',emoji:'📜',cost:350,sell:210,gradient:'linear-gradient(135deg,#3c2b20,#9b7148,#d0ae76)',glow:'rgba(218,174,111,.20)',desc:'Collector-value treatment.',effect:'All 5 cards receive a permanent Vintage treatment worth +35% collector value.'},
    secret:{id:'secret',name:'Secret Hunt Pack',emoji:'👁️',cost:1200,sell:720,gradient:'linear-gradient(135deg,#f4f6ff,#797f92,#11141b)',glow:'rgba(255,255,255,.24)',desc:'A dangerous Secret chase.',effect:'Each card gets a greatly increased Secret roll. Still not guaranteed.'},
    graded:{id:'graded',name:'Graded Pack',emoji:'🏷️',cost:700,sell:420,gradient:'linear-gradient(135deg,#183e4b,#4db8c3,#d8f7f5)',glow:'rgba(102,231,239,.22)',desc:'Cards arrive slabbed.',effect:'Every card is automatically graded from 7.0–10.0 when opened.'}
  };
  const themeDefs=[
  ...
  const defaultState=()=>({
    ...
    adminForces:{godMode:null,hotNext:false,bonusNext:false,ghostNext:false,blackLabelNext:false,glitchNext:false,crueNext:false},
  ...
  const upgrades={
    ...
    campaignluck:{name:'Luckier Campaigns',desc:'Raises the odds of bonus rewards, potions, specials, and hidden-set finds.',base:20,growth:1.30,key:'engineLevel'}
  };
  const potionDefs={
  ...
  function overdriveActive(){return !!(state.overdrive&&Number(state.overdrive.activeUntil)>Date.now())}
  function overdriveBoost(){return overdriveActive()?5:1}
  ...
  function addOverdriveProgress(kind,count=1){
    if(tutorialTrainingMode||overdriveActive())return false;state.overdrive=state.overdrive||{points:0,clickRemainder:0,packRemainder:0,activeUntil:0,totalLaunches:0};count=Math.max(0,Math.floor(Number(count)||0));if(!count)return false;const before=state.overdrive.points||0;
    if(kind==='click'){state.overdrive.clickRemainder=(state.overdrive.clickRemainder||0)+count;const gained=Math.floor(state.overdrive.clickRemainder/75);state.overdrive.clickRemainder%=75;if(gained)state.overdrive.points=Math.min(100,(state.overdrive.points||0)+gained)}
    else if(kind==='pack'){state.overdrive.packRemainder=(state.overdrive.packRemainder||0)+count;const gained=Math.floor(state.overdrive.packRemainder/3);state.overdrive.packRemainder%=3;if(gained)state.overdrive.points=Math.min(100,(state.overdrive.points||0)+gained)}
    else if(kind==='grade')state.overdrive.points=Math.min(100,(state.overdrive.points||0)+count);
    else if(kind==='campaign')state.overdrive.points=Math.min(100,(state.overdrive.points||0)+count*3);
    if((state.overdrive.points||0)!==before){markSaveDirty();save();renderOverdrive();if(state.overdrive.points>=100&&before<100){sfx('grade');toast('🔥 OVERDRIVE READY','The meter is full. Press LAUNCH OVERDRIVE when you are ready.')}}return (state.overdrive.points||0)!==before
  }
  ...
  if($('#overdriveLaunch'))$('#overdriveLaunch').onclick=launchOverdrive;setInterval(()=>{if(document.hidden)return;renderOverdrive();if(pfAdminUnlocked&&!$('#gameSettingsModal').classList.contains('hidden'))renderPfAdminBoard()},1000);setTimeout(renderOverdrive,0);
  ...
  const sellBaseByRarity={Common:8,Uncommon:12,Rare:20,Epic:40,Legendary:100};
  ...
  function grantOwnerMegaVault(){
    ...
    for(const id of Object.keys(premiumPackDefs))grantPremiumPack(id,count);
    const pool=basePackThemes(),muts=Object.values(packMutations);
    for(const t of pool){
      for(const m of muts){
        for(let i=0;i<count;i++)state.specialPackInventory.push({uid:uid(),themeId:t.id,mutation:m.id});
      }
    }
  ...
  const rewardCodeDefs={
    "SANSAEVANS":{label:'OWNER VAULT — $100,000,000 + 1,000 of every pack + 1,000 of every potion',apply:()=>grantOwnerMegaVault(),rewardId:"SANSAEVANS"},
  ...
  const PF_ADMIN_PASSWORD_B64='U0FOU0FFVkFOUw==';
  ...
  function unlockPfAdmin(){const input=$('#pfAdminPassword');let expected='';try{expected=atob(PF_ADMIN_PASSWORD_B64)}catch(e){}if(String(input?.value||'')===expected){pfAdminUnlocked=true;if(input)input.value='';setPfAdminStatus('');setPfAdminStatus('Admin board unlocked.',true,true);renderPfAdminBoard();sfx('buy')}else{setPfAdminStatus('Incorrect admin password.');sfx('error')}}
  ...
  function setBagTab(tab){if(!['cards','packs','potions'].includes(tab))tab='cards';currentBagTab=tab;$$('[data-bag-tab]').forEach(b=>b.classList.toggle('active',b.dataset.bagTab===tab));$('#bagCardsTab').classList.toggle('hidden',tab!=='cards');$('#bagPacksTab').classList.toggle('hidden',tab!=='packs');$('#bagPotionsTab').classList.toggle('hidden',tab!=='potions');if(tab==='cards')renderCollection();if(tab==='packs')renderPacks();if(tab==='potions')renderPotions();updateBagCounts()}
  ...
  function buyPremiumPack(id){const d=premiumPackDefs[id];if(!d||!spend(d.cost)){sfx('error');renderPremiumPackShop();return}state.premiumPackInventory[id]=(state.premiumPackInventory[id]||0)+1;state.stats.premiumPacksBought=(state.stats.premiumPacksBought||0)+1;sfx('buy');toast('Specialty pack sent to Bag',`${d.name} is waiting in Bag → Packs.`);save();renderHUD();renderShop();updateBagCounts();if(!$('#view-collection').classList.contains('hidden')&&currentBagTab==='packs')renderPacks()}
  ...
  function pickRarity(autoMode=false,qualityBoost=1){
    ...
    if(Math.random()<packQualityPromoteChance()*(autoMode?.5:1)*qualityBoost){const i=rarityOrder.indexOf(result.rarity);if(i>=0&&i<rarityOrder.length-1){result={rarity:rarityOrder[i+1],nearMiss:null}}}return result
  }
  function weightedCard(theme,mutation,autoMode=false,qualityBoost=1){
    if(!tutorialTrainingMode&&Math.random()<GHOST_CARD_CHANCE*(autoMode?.5:1)*overdriveBoost()){state.quirkStats.ghostCards=(state.quirkStats.ghostCards||0)+1;return {card:GHOST_CARD,nearMiss:null}}
    if(Math.random()<SECRET_CHANCE*(autoMode?.5:1)*overdriveBoost())return {card:theme.secretCards[Math.floor(Math.random()*theme.secretCards.length)],nearMiss:null};
  ...
  function rollPackVariantName(autoMode=false,qualityBoost=1){
    const luck=variantLuckMultiplier(),autoPenalty=autoMode?.5:1,raw=Math.random();let cumulative=0,previous=null,previousChance=0;
  ...
  function pullScore(p){return (p.card.secret?1000:rarityOrder.indexOf(p.card.rarity)*100)+effectiveCardValue(p.card,1,p.copy)/1000}
  ...
  function premiumVariantName(autoMode=false){const once=()=>{const scale=autoMode?.5:1,r=Math.random();let c=.01*scale;if(r<c)return'Serialized';c+=.10*scale;if(r<c)return'Prismatic';c+=.06*scale;if(r<c)return'Negative';c+=.10*scale;if(r<c)return'Gold';c+=.15*scale;if(r<c)return'Holo';return'Foil'},rank={Foil:0,Holo:1,Gold:2,Negative:3,Prismatic:4,Serialized:5};let best=once();if(overdriveActive())for(let i=1;i<5;i++){const v=once();if(rank[v]>rank[best])best=v}return best}
  function premiumRareCard(theme,autoMode=false){const r=Math.random();let rarity=autoMode?(r<.80?'Rare':r<.92?'Epic':r<.975?'Legendary':r<.9935?'Mythic':r<.9985?'Divine':'Ultra'):(r<.60?'Rare':r<.84?'Epic':r<.95?'Legendary':r<.987?'Mythic':r<.997?'Divine':'Ultra');const pool=theme.cards.filter(c=>c.rarity===rarity);return (pool.length?pool:theme.cards.filter(c=>rarityOrder.indexOf(c.rarity)>=2))[Math.floor(Math.random()*Math.max(1,(pool.length?pool:theme.cards).length))]||theme.cards[0]}
  ...
  function createPremiumPackPulls(def,theme,autoMode=false){
    ...
    if(def.id==='secret'){for(const p of pulls)if(!p.card.secret&&Math.random()<.004*(autoMode?.5:1)){const c=theme.secretCards[Math.floor(Math.random()*theme.secretCards.length)];swapPremiumPullCard(p,c)}}
  ...
  function createPackPulls(t,mutation,autoMode=false){
    ...
    const forcedGodMode=!tutorialTrainingMode&&['rarity','variant'].includes(af.godMode)?af.godMode:null;
    const godPack=!tutorialTrainingMode&&(!!forcedGodMode||Math.random()<GOD_PACK_CHANCE*overdriveBoost()),godMode=forcedGodMode||(godPack?(Math.random()<.5?'rarity':'variant'):null);
    if(forcedGodMode)af.godMode=null;
    if(godPack)state.quirkStats.godPacks=(state.quirkStats.godPacks||0)+1;
    let packSize=5,anomaly=null;
    if(!tutorialTrainingMode&&!godPack){
    ...
    const godSerializedSlot=godPack&&godMode==='variant'?Math.floor(Math.random()*packSize):-1;
    for(let i=0;i<packSize;i++){
      const cardMutation=specialSlots.has(i)?mutation:null,forceGhostHere=forceGhost&&i===packSize-1,rolled=forceGhostHere?{card:GHOST_CARD,nearMiss:null}:(godPack&&godMode==='rarity'?{card:godCard(i===packSize-1),nearMiss:null}:godPack&&godMode==='variant'?{card:godVariantCard(),nearMiss:null}:weightedCard(t,cardMutation,autoMode,qualityBoost)),card=rolled.card;
      if(forceGhostHere)state.quirkStats.ghostCards=(state.quirkStats.ghostCards||0)+1;
      let variant=godPack&&godMode==='variant'?(i===godSerializedSlot?'Serialized':guaranteedVariant()):(i===variantSlot?packVariant:'Normal');const serial=variant==='Serialized'?makeSerial(card.id):null,copy=makeCopy(variant,cardMutation?.id||null,serial);maybeApplyCollectorQuirks(copy,card);
      const isNew=!seen.has(card.id);seen.add(card.id);pulls.push({card,copy,newDiscovery:isNew,added:false,special:!!cardMutation,nearMiss:rolled.nearMiss||(i===variantSlot?vr.nearMiss:null),godPack:!!godPack})
    }
    pulls.sort((a,b)=>pullScore(a)-pullScore(b));if(packSize===6&&pulls.length)pulls[pulls.length-1].stuckExtra=true;
    ...
    pulls._godPack=godPack;pulls._godMode=godMode;pulls._anomaly=anomaly;pulls._hotBoxReveal=hotReveal;pulls._hotBoxTheme=t.name;
  ...
  function showPackOdds(id){
    const t=themes.find(x=>x.id===id),m=$('#oddsModal');if(!t)return;m.classList.remove('hidden');
    const pct=p=>{const n=p*100;return n>=1?n.toFixed(1)+'%':n>=.1?n.toFixed(2)+'%':n>=.01?n.toFixed(3)+'%':n.toFixed(4)+'%'};const packOneIn=p=>Math.max(1,Math.round(1/(1-Math.pow(1-p,5))));
    const rarityRows=rarityOrder.map(r=>`<div class="odds-card-row"><i style="--c:${rarityColor[r]}"></i><div><b>${r}</b><small>${['Legendary','Mythic','Divine','Ultra'].includes(r)?`about 1 in ${packOneIn(RARITY_ODDS[r]).toLocaleString()} packs`:'per card'}</small></div><strong>${pct(RARITY_ODDS[r])}</strong></div>`).join('')+`<div class="odds-card-row secret"><i style="--c:#fff"></i><div><b>Secret Tier</b><small>1 in 100,000 cards · about 1 in ${packOneIn(SECRET_CHANCE).toLocaleString()} 5-card packs</small></div><strong>${pct(SECRET_CHANCE)}</strong></div><div class="odds-card-row ghost-tier-odds"><i style="--c:${rarityColor.Ghost}"></i><div><b>Ghost Tier</b><small>1 in 1,000,000 cards · about 1 in ${packOneIn(GHOST_CARD_CHANCE).toLocaleString()} 5-card packs</small></div><strong>${pct(GHOST_CARD_CHANCE)}</strong></div>`;
    const variantLuck=variantLuckMultiplier();const variantRows=variantRollOrder.slice().reverse().map(v=>{const chance=variantDefs[v].chance*(v==='Foil'?1:variantLuck);return `<div class="odds-pill"><span>${v}</span><b>${pct(chance)}</b></div>`}).join('');
    const mutationRows=Object.values(packMutations).map(x=>`<div class="mutation-odds"><b>${x.name}</b><span>${x.desc}</span></div>`).join('');
    m.innerHTML=`<div class="modal odds-modal odds-redesign fade-in"><div class="odds-hero" style="background:${t.gradient}"><div class="odds-mini-pack" style="background:${t.gradient}"><span>PACKFORGE</span><b>${t.name}</b><small>5 CARD PACK</small></div><div><small>PACK ODDS</small><h2>${t.name}</h2><p>Five cards per pack. Your best rolled rarity is shown fifth.</p></div><button class="close-x" id="closeOdds">×</button></div><div class="odds-body"><section><h3>Rarity odds <span>per card</span></h3><p class="odds-explain">Manual opening uses the odds below. Auto Open uses half the base chance for Rare+ cards and also halves both Secret and Ghost tier rolls.</p><div class="odds-rarity-grid">${rarityRows}</div></section><section><h3>Variants <span>per pack · max 1</span></h3><p class="odds-explain">Standard variants are rolled once for the whole pack, so five separate card rolls can no longer stack the odds. Foil is fixed at 25% for manual openings. Auto Open halves every standard variant chance. Variant-luck bonuses improve the rarer premium variants instead, and are capped to prevent variant flooding.</p><div class="odds-pill-grid">${variantRows}</div></section><section><h3>Special packs</h3><p class="odds-explain">About ${(specialPackChance()*100).toFixed(2)}% of normal pack openings mutate. A special pack gives its treatment to <b>only 1–3 random cards</b> inside, not all five.</p><div class="mutation-odds-grid">${mutationRows}</div></section></div></div>`;
    $('#closeOdds').onclick=()=>m.classList.add('hidden');m.onmousedown=e=>{if(e.target===m)m.classList.add('hidden')}
  }
  ...
  function openPackAnimation(theme,pulls,mutation){
    const ov=$('#packOverlay');ov.classList.remove('hidden');const special=mutation?`<span class="mutation-head">${mutation.name}</span>`:'';
    ov.innerHTML=`<div class="opening-title"><h2>${theme.name} Pack ${special}</h2><p class="${mutation?'special-pack-callout':''}">${mutation?`SPECIAL PACK · 1–3 cards inside will carry the ${mutation.name} treatment.`:'Start at either edge and cut all the way across the faint line.'}</p></div><div class="rip-stage"><div class="big-pack premium-wrapper sleek-wrapper ${mutation?`special-pack special-${mutation.id}`:''}" id="bigPack" style="background:${theme.gradient};color:white"><div class="pack-rip-strip" style="background:${theme.gradient}"></div><div class="rip-line"></div><canvas class="cut-path-canvas" id="cutPathCanvas"></canvas><div class="seam-cursor" id="seamCursor"></div><div class="seam-instruction">HOLD + CUT FROM ONE EDGE TO THE OTHER</div><div class="pack-logo sleek-pack-logo"><div class="pack-theme-mark">${theme.emoji}</div><b>${theme.name} Pack</b>${mutation?`<span class="sleek-special-name">${mutation.name.toUpperCase()} SPECIAL PACK</span>`:`<span class="sleek-standard-name">PACKFORGE</span>`}<small>5 TRADING CARDS</small></div></div></div>`;
    const pack=$('#bigPack'),canvas=$('#cutPathCanvas'),cursor=$('#seamCursor');
    ...
    pack.addEventListener('pointerdown',e=>{if(finished)return;const r=pack.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top;seamY=Math.max(55,Math.min(82,r.height*.145));const edge=Math.max(28,r.width*.11);if((x>edge&&x<r.width-edge)||y<28||y>r.height-24){sfx('error');return}startSide=x<=edge?'left':'right';dragging=true;points=[{x,y}];pack.classList.add('dragging');pack.setPointerCapture(e.pointerId);cursor.style.opacity='1';cursor.style.left=x+'px';cursor.style.top=(y-12)+'px';drawCut();e.preventDefault()});
  ...
  function sortEntries(a,b){return Number(!!state.favorites[b.c.id])-Number(!!state.favorites[a.c.id])||sellValue(b.c,b.level,b.rep)-sellValue(a.c,a.level,a.rep)||effectiveCardValue(b.c,b.level,b.rep)-effectiveCardValue(a.c,a.level,a.rep)||Number(b.c.secret)-Number(a.c.secret)||(b.c.crue?999:rarityOrder.indexOf(b.c.rarity))-(a.c.crue?999:rarityOrder.indexOf(a.c.rarity))||b.level-a.level||(b.rep.grade||0)-(a.rep.grade||0)||a.c.name.localeCompare(b.c.name)}
...
(function(){
  ...
  let coinBusy=false;
  function coinFlip(){if(coinBusy)return;const bet=parseBet($m('coinBetInput')?.value);if(!(bet>=0))return alertMsg('Invalid Bet','Enter a valid bet of $0.00 or more.');if(bet>state.cash)return alertMsg('Not enough cash',`You only have ${fmt(state.cash)}.`);if(!spendCash(bet))return;coinBusy=true;$m('coinFlipBtn').disabled=true;$m('coinFlipResult').textContent='Flipping...';$m('coin')?.classList.add('flipping');setTimeout(()=>{const win=Math.random()<.5;$m('coin')?.classList.remove('flipping');let ret=0;if(win){ret=roundMoney(bet*2);payCash(ret);$m('coinFlipResult').textContent=`HEADS — WIN +${fmt(bet)}`;try{tone(620,.12,'sine',.018)}catch(e){}}else{$m('coinFlipResult').textContent=`TAILS — LOSE -${fmt(bet)}`;try{tone(150,.11,'sine',.014)}catch(e){}}recordGamble('coinflip',bet,ret);coinBusy=false;$m('coinFlipBtn').disabled=false;sync()},900)}
...
function renderCasino(){try{mccInitCasino()}catch(err){console.error('Meme casino render error',err)}const cash=$('#casinoCashReadout');if(cash)cash.textContent=fmt(state.cash)}
  ...
  function campaignAddPack(themeId=null,special=false){const pool=basePackThemes();const t=themeId?themes.find(x=>x.id===themeId):pool[Math.floor(Math.random()*Math.max(1,pool.length))];if(!t)return null;if(special){const muts=Object.values(packMutations),m=muts[Math.floor(Math.random()*muts.length)];const pack={uid:uid(),themeId:t.id,mutation:m.id,source:'Exhibition Campaign'};state.specialPackInventory.push(pack);return {theme:t.name,special:m.name}}state.packInventory[t.id]=(state.packInventory[t.id]||0)+1;return {theme:t.name,special:null}}
  ...
  function campaignRewardText(def){const bits=[];if(def.packs)bits.push(`${def.packs} pack${def.packs===1?'':'s'}`);if(def.packChance)bits.push(`${Math.round(def.packChance*100)}% bonus pack`);bits.push(fmt(def.cashReward));if(def.special)bits.push(`${Math.round(def.special*100)}% special`);if(def.potionChance)bits.push(`${Math.round(def.potionChance*100)}% potion`);return bits.join(' · ')}
  ...
  function showCampaignRewardPopup(summary){
    ...
    m.innerHTML=`<div class="modal campaign-reward-modal fade-in"><div class="campaign-reward-burst">CAMPAIGN COMPLETE</div><h2>${summary.name}</h2><div class="campaign-reward-tier">${summary.tier} TIER · ${summary.power.toLocaleString()} POWER · ${summary.speed.toFixed(summary.speed>=10?0:1)}× SPEED</div><div class="campaign-reward-list"><div class="campaign-reward-line"><span>Cash reward</span><b>${fmt(summary.cash)}</b></div>${packRows||'<div class="campaign-reward-line"><span>Packs</span><b>None this run</b></div>'}${summary.potionWon?`<div class="campaign-reward-line potion-win"><span>${summary.potionWon.emoji} ${summary.potionWon.name}</span><b>POTION ×1</b></div>`:''}${summary.hiddenSetWon?`<div class="campaign-reward-line hidden-set-win"><span>${summary.hiddenSetWon.emoji} Hidden set discovered: ${summary.hiddenSetWon.name}</span><b>10 SHOP BUYS</b></div>`:''}</div><button class="campaign-reward-done" id="campaignRewardDone">Collect & See New Offers</button></div>`;
  ...
  function collectCampaignReward(){
    ...
    let potionWon=null;const potionChance=Math.min(.95,((def.potionChance??fallbackRewards.potionChance??0)+(tier.name==='Master'?.05:tier.name==='Gold'?.025:0))*od);if(Math.random()<potionChance){const ids=Object.keys(potionDefs),id=ids[Math.floor(Math.random()*ids.length)];addPotion(id,1);potionWon=potionDefs[id]}
  ...
  function campaignOfferRewardCards(def){
    ...
    if(def.potionChance)out.push(`${Math.round(def.potionChance*100)}% potion chance`);
  ...
  const eventDefs=[
    ...
    {id:'collectorbuyout',name:'Collector Buyout',icon:'💵',weight:7,rarity:'Rare',desc:'Card sell values are 10% higher for five minutes. The bonus is intentionally modest so bulk selling is useful without breaking the economy.'},
    {id:'potioncart',name:'Potion Cart',icon:'🧪',weight:8,rarity:'Uncommon',desc:'A traveling alchemist offers three one-time potion purchases at a discount.'},
    {id:'mysteryparcel',name:'Mystery Parcel',icon:'📦',weight:8,rarity:'Uncommon',desc:'Pay $150 once to open a sealed parcel containing cash, a potion, or occasionally a pack.'},
  ...
  const ACHIEVEMENTS=[
    ...
    {id:'first_potion',icon:'🧪',name:'First Sip',desc:'Use any potion.',cat:'Potions',tier:'Starter',check:s=>Object.keys(s.meta.potionsUsed).length>=1},
    {id:'all_potions',icon:'⚗️',name:'Full Medicine Cabinet',desc:'Use every potion type at least once.',cat:'Potions',tier:'Hard',check:s=>Object.keys(potionDefs).every(id=>!!s.meta.potionsUsed[id])},
    {id:'triple_effect',icon:'✨',name:'Triple Effect',desc:'Have three different potion effects active at the same time.',cat:'Potions',tier:'Advanced',check:s=>(s.meta.maxActivePotions||0)>=3},
    {id:'first_event',icon:'✦',name:'Event Horizon',desc:'See your first random live event.',cat:'Events',tier:'Starter',check:s=>(state.stats.eventsSeen||0)>=1},
  ...
  function checkAchievements(notify=true){
    ...
    if(unlockedAchievementCount()===100&&!ach.masterRewardClaimed)awardAchievementMastery();return newOnes.length
  }
  ...
  setInterval(()=>{if(document.hidden)return;const a=activeCampaign();if(a&&Date.now()>=a.endsAt&&!a.notified){a.notified=true;save();sfx('grade');toast('Campaign complete',`${a.offer?.name||'Exhibition'} is ready to collect.`)}if(!$('#view-campaigns').classList.contains('hidden')){if(a)renderCampaignActive();else updateCampaignRefreshButton()}},1000);
  ...
  /* ==========================================================
     ...
     ========================================================== */
  const PF_UPDATE_ID='secret-ghost-tiers-20260910';
  const PF_VERSION='2.11';
  ...
  collectionEntries=function(){
    ...
    for(const c of [GHOST_CARD,PF_CRUE_LOWE_CARD]){
  ...
  createPackPulls=function(t,mutation,autoMode=false){
    ...
    if(forceCrue)af.crueNext=false;if(forceGlitch)af.glitchNext=false;
    ...
    if(glitchPack&&pulls.length){
      const eligible=pulls.map((p,i)=>({p,i})).filter(x=>!x.p.card.crue);if(eligible.length){const hit=eligible[Math.floor(Math.random()*eligible.length)].p;hit.copy.variant='Glitched';hit.copy.serial=null;const alts=t.cards.filter(c=>c.id!==hit.card.id);hit.copy.glitchTarget=(alts[Math.floor(Math.random()*Math.max(1,alts.length))]||t.cards[0]||hit.card).name;state.quirkStats.glitchedCards=(state.quirkStats.glitchedCards||0)+1;}
    }
  ...
  sellValue=function(card,level,copy){if(card?.crue)return .01;return pfBaseSellValue(card,level,copy)};
  ...
  function pfCollectCampaignSlot(slot){
    ...
    const tier=campaignTier(def,a.power),fallbackRewards=campaignRewardsForMinutes(def.baseMinutes||def.minutes||20,def.difficulty||'standard'),od=overdriveBoost(),cash=Math.round((def.cashReward??fallbackRewards.cash)*(1+Math.min(.35,(a.power/def.power-1)*.025))*od);if(cash>0)earn(cash);
    const packRewards=[];let packCount=((def.packs||0)+tier.bonusPacks)*od;for(let i=0;i<packCount;i++){const p=campaignAddPack(null,false);if(p)packRewards.push(p)}const extraChance=Math.min(.98,((def.packChance||0)+(tier.extraPackChance||0))*od);if(Math.random()<extraChance){const p=campaignAddPack(null,false);if(p)packRewards.push(p)}let specialWon=false;const specialChance=Math.min(.90,((def.special||0)+(tier.specialBonus||0))*od);if(Math.random()<specialChance){const p=campaignAddPack(null,true);if(p){packRewards.push(p);specialWon=true}}let potionWon=null;const potionChance=Math.min(.95,((def.potionChance??fallbackRewards.potionChance??0)+(tier.name==='Master'?.05:tier.name==='Gold'?.025:0))*od);if(Math.random()<potionChance){const ids=Object.keys(potionDefs),id=ids[Math.floor(Math.random()*ids.length)];addPotion(id,1);potionWon=potionDefs[id]}const hiddenSetWon=tryUnlockCampaignSet();state.campaigns.completed.total=(state.campaigns.completed.total||0)+1;addOverdriveProgress('campaign',1);state.campaigns[key]=null;state.stats.campaignsCompleted=(state.stats.campaignsCompleted||0)+1;state.stats.campaignCash=(state.stats.campaignCash||0)+cash;state.stats.campaignPacksWon=(state.stats.campaignPacksWon||0)+packRewards.filter(p=>!p.special).length;state.stats.campaignSpecialsWon=(state.stats.campaignSpecialsWon||0)+(specialWon?1:0);state.stats.campaignPotionsWon=(state.stats.campaignPotionsWon||0)+(potionWon?1:0);const summary={name:def.name,tier:tier.name,power:a.power,cash,packs:packRewards,specialWon,potionWon,hiddenSetWon,speed:a.speed||campaignSpeed(def,a.power)};generateCampaignOffers();sfx('grade');confetti(tier.name==='Master'?28:14);save();renderAll();renderCampaigns();showCampaignRewardPopup(summary)
  }
  ...
  function pfRenderChances(){
    ...
    const chanceIntro=$('#stats-chances .pf-chances-head p');if(chanceIntro)chanceIntro.textContent='Base/manual odds plus the major random systems across packs, cards, Coin clicks, grading, live events, Mystery Parcels, and jackpots. Auto Open and Overdrive modifiers are noted where they change the roll.';
    ...
    add('SECRET TIER','Secret Card',pfPackProb(SECRET_CHANCE),'packs','Secret tier is 1 in 100,000 cards before modifiers; about 1 in 20,000 normal 5-card packs. Auto Open halves the per-card Secret roll.');
    add('GHOST TIER','Ghost Card',pfPackProb(GHOST_CARD_CHANCE),'packs','Separate Ghost rarity: 1 in 1,000,000 cards before modifiers; about 1 in 200,000 normal 5-card packs. Auto Open halves it. Huge sell value.');
    add('HIDDEN CARDS','Crue Lowe',PF_CRUE_PACK_CHANCE,'packs','One fabled card; Grade 1; $0.01 sell value.','', 'crue');
    for(const v of ['Foil','Holo','Gold','Negative','Prismatic','Serialized'])add('STANDARD VARIANTS',v,variantDefs[v].chance||0,'packs','Rolled once per normal pack (max one standard variant). Auto Open halves standard variant odds.');
    ...
    add('PACK ODDITIES','Special Pack mutation',SPECIAL_PACK_CHANCE,'packs','Base normal-pack mutation chance before event/showcase/Overdrive luck.');
    ...
    add('PACK ODDITIES','God Pack',GOD_PACK_CHANCE,'packs','Either God Pack mode. The rarity version is now all Epic+ with a Mythic+ final card; the variant version is Rare+ with premium variants and one guaranteed Serialized.');
    add('PACK ODDITIES','Epic+ God Pack',GOD_PACK_CHANCE*.5,'packs','Half of natural God Packs. Every card is Epic+; final card is Mythic+.');
    add('PACK ODDITIES','Variant God Pack',GOD_PACK_CHANCE*.5,'packs','Half of natural God Packs. Every card is Rare+ and premium variant; one slot is guaranteed Serialized.');
    ...
    add('COIN','Free Pack drop',packDropChance(),'clicks','Current Coin-click pack-drop chance. Base is 1 in 200; Overdrive increases positive luck.');
    ...
    add('FLOATING COIN','Cash + bonus pack jackpot',.05,'clicked floating coins','Awards a large cash burst plus one random premium pack.');
    ...
    add('GRADING','Natural 10.0 roll',.04,'grade rolls','Before opening/factory damage penalties. Overdrive rolls multiple times and keeps the best.');
    ...
    add('MYSTERY PARCEL','Potion',.20,'parcels','Any potion type can be selected.');
    add('MYSTERY PARCEL','Cash',.62,'parcels','$45–$230 cash.');
  ...
  function pfRenderJournal(){const g=$('#pfJournalGrid');if(!g)return;const hasSecret=allCards.some(c=>c.secret&&state.history?.[c.id]?.totalPulled>0),q=state.quirkStats||{},events=[['🌟','God Pack',q.godPacks||0,'A wrapper where every card broke the normal rules.'],['🔥','Hot Box',q.hotBoxes||0,'Three unusually loaded packs from one set.'],['👻','Ghost / Unlisted',q.ghostCards||0,`${pfOwnedCount(GHOST_CARD.id)} currently owned.`],['🧩','Glitched Variant',q.glitchedCards||0,'A card caught cleanly forming toward another print.'],['📦','Factory Pack Anomaly',q.packAnomalies||0,'A four- or six-card wrapper.'],['⚠','Factory Error Card',q.factoryErrors||0,'A true production defect with collector consequences.'],['✦','Hidden Holo Pattern',q.holoPatterns||0,'Swirl, Starburst, Galaxy, or Double Holo.'],['🏷','Perfect Black Label',q.blackLabels||0,'The rarest normal grading result.'],['👁','Secret Card',hasSecret?1:0,'A card hidden outside the normal 100-card checklist.'],['♜','Crue Lowe',q.crueLowe||0,`${pfOwnedCount(PF_CRUE_LOWE_CARD.id)} currently owned.`]];g.innerHTML=events.map(([icon,name,count,desc])=>{const seen=count>0;return `<article class="pf-journal-entry ${seen?'seen':'locked'}"><div class="pf-journal-icon">${seen?icon:'?'}</div><div><small>${seen?'DISCOVERED':'UNDISCOVERED'}</small><b>${seen?name:'UNDISCOVERED'}</b><span>${seen?desc:'Keep playing. PackForge does not reveal what is missing.'}</span></div>${seen?`<strong>×${Number(count).toLocaleString()}</strong>`:''}</article>`}).join('')}
  const pfBaseRenderStats=renderStats;renderStats=function(){pfBaseRenderStats();pfRenderChances();pfRenderJournal();const rs=$('#recordStatsGrid');if(rs){const ghostOwned=pfOwnedCount(GHOST_CARD.id),crueOwned=pfOwnedCount(PF_CRUE_LOWE_CARD.id);rs.insertAdjacentHTML('beforeend',`<div class="stat-tile"><small>Ghost Cards Owned</small><b>${ghostOwned.toLocaleString()}</b></div><div class="stat-tile"><small>Glitched Pulls</small><b>${(state.quirkStats.glitchedCards||0).toLocaleString()}</b></div><div class="stat-tile"><small>Crue Lowe Owned</small><b>${crueOwned.toLocaleString()}</b></div><div class="stat-tile"><small>Overdrives Launched</small><b>${(state.overdrive.totalLaunches||0).toLocaleString()}</b></div>`)} };
  ...
  function pfShowWhatsNew(){pfClearUpdateBadge();const m=$('#confirmModal');m.classList.remove('hidden');m.innerHTML=`<div class="modal pf-whats-new fade-in"><div class="modal-head"><h3>PackForge v${PF_VERSION}</h3><button class="close-x" id="pfWhatsNewClose">×</button></div><div class="modal-body"><div class="pf-update-kicker">COLLECTOR SYSTEMS UPDATE</div><h2>A lot changed.</h2><div class="pf-change-grid"><span>🧩 Glitched variant · 1 in 50 packs</span><span>◇ Two simultaneous grading slots</span><span>✦ Two simultaneous campaigns</span><span>▥ Chances page in exact pack terms</span><span>📖 Rare Event Journal + expanded records</span><span>💾 Export, import, and rotating save recovery</span><span>! Admin card editor from Inspect</span><span>♜ Crue Lowe · 1 in 500,000,000 packs</span><span>⚡ Automatic low-FPS protection</span><span>🪙 Full reactor Coin effects · combo heat · crit lightning · jackpots</span><span>🎰 $0.00 no-stakes casino play</span></div><button class="grade-action" id="pfWhatsNewDone">Got it</button></div></div>`;const close=()=>{m.classList.add('hidden');m.innerHTML=''};$('#pfWhatsNewClose').onclick=close;$('#pfWhatsNewDone').onclick=close}
  ...
  function pfGuardLoop(now){const gameVisible=!$('#gameApp')?.classList.contains('hidden')&&document.visibilityState==='visible';if(!gameVisible){pfGuardFrames=0;pfGuardStart=now;requestAnimationFrame(pfGuardLoop);return}pfGuardFrames++;if(!pfGuardTriggered&&state.settings.autoPerformanceGuard&&now-pfGuardStart>=8000){const fps=pfGuardFrames*1000/(now-pfGuardStart);if(fps<35){pfGuardTriggered=true;state.settings.antiLag=state.settings.antiLag==='off'?'balanced':state.settings.antiLag;state.settings.particles=state.settings.particles==='full'?'reduced':state.settings.particles;state.settings.quality=state.settings.quality==='high'?'balanced':state.settings.quality;applyPerformanceSettings();syncSettingsUI();save();toast('Performance Guard enabled',`PackForge detected about ${Math.round(fps)} FPS and trimmed expensive visual effects. Odds are unchanged.`)}pfGuardFrames=0;pfGuardStart=now}requestAnimationFrame(pfGuardLoop)}requestAnimationFrame(pfGuardLoop);
  ...
  const rf=$('#rarityFilter');if(rf&&!Array.from(rf.options).some(o=>o.value==='Crue Lowe')){const o=document.createElement('option');o.value='Crue Lowe';o.textContent='Crue Lowe';rf.appendChild(o)}
  ...
  /* Optional cloud-save bridge. This does not alter gameplay or the local save format. */
  window.PackForgeBridge={
    getState:()=>state,
    getSnapshot:()=>JSON.parse(JSON.stringify(state,compactSaveReplacer)),
    saveNow:()=>save(true),
    requestSave:()=>save(),
    renderAll:()=>renderAll(),
    toast:(a,b)=>toast(a,b),
    saveKey:()=>saveKey,
    schemaVersion:()=>SAVE_SCHEMA,
    replaceSaveAndReload:(obj,source='external')=>{if(!obj||typeof obj!=='object'||Array.isArray(obj)||obj.cash===undefined||!obj.inventory)return false;try{if(typeof pfSnapshotCurrent==='function')pfSnapshotCurrent(true);localStorage.setItem('packforge_cloud_restore_source',source);localStorage.setItem(saveKey,JSON.stringify(obj));location.reload();return true}catch(e){console.warn('External save restore failed',e);return false}},
    backupCurrent:()=>{try{return typeof pfSnapshotCurrent==='function'?pfSnapshotCurrent(true):false}catch(e){return false}}
  };
  try{window.dispatchEvent(new CustomEvent('pf:bridge-ready'))}catch(e){}
})();