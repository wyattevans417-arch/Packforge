...
(() => {
  ...
  function serialSpecialInfo(copy){
    if(copy?.variant!=='Serialized'&&!copy?.serial)return null;const m=String(copy?.serial||'').match(/#(\d+)/),n=m?+m[1]:0;if(!n)return null;const p=String(n).padStart(3,'0');
...
function renderCasino(){try{mccInitCasino()}catch(err){console.error('PackForge casino render error',err)}const cash=$('#casinoCashReadout');if(cash)cash.textContent=fmt(state.cash)}
  ...
  const ACHIEVEMENTS=[
    ...
    {id:'serial_50',icon:'#50',name:'Top Fifty',desc:'Own a Serialized card numbered #50 or better.',cat:'Serialized',tier:'Hard',check:s=>s.copies.some(x=>!!x.copy.serial&&serialNumber(x.copy)<=50)},
    {id:'serial_10',icon:'#10',name:'Top Ten',desc:'Own a Serialized card numbered #10 or better.',cat:'Serialized',tier:'Brutal',check:s=>s.copies.some(x=>!!x.copy.serial&&serialNumber(x.copy)<=10)},
    {id:'serial_one',icon:'#1',name:'Number One',desc:'Own Serialized card #1.',cat:'Serialized',tier:'Secret',check:s=>s.copies.some(x=>!!x.copy.serial&&serialNumber(x.copy)===1)},
    {id:'serialized_secret',icon:'✧#',name:'The White Number',desc:'Own a Serialized Secret card.',cat:'Serialized',tier:'Secret',check:s=>s.copies.some(x=>x.card.secret&&!!x.copy.serial)},
  ...
  renderShop=function(){
    const grid=document.getElementById('shopGrid');if(!grid)return;const arr=themes.filter(shopThemeVisible),note=document.getElementById('packTierNote');if(note)note.textContent='10 permanent sets · 250 cards each. Use the ? button for exact pack odds and differences.';
    grid.innerHTML=arr.map(t=>{const price=packPrice(t);return `<article class="shop-offer"><div class="shop-offer-art" style="background:${t.gradient}"><div class="shop-mini-pack" style="background:${t.gradient}"><span class="shop-pack-emoji">${t.emoji}</span><span class="shop-pack-name">${t.name.toUpperCase()}</span><small>5 CARDS · 250 CARD SET</small></div></div><div class="shop-offer-copy"><h3>${t.name} Pack</h3><p>${t.desc}</p><div class="shop-price">${fmt(price)}</div><button class="shop-buy" data-shop-buy="${t.id}" ${canAfford(price)?'':'disabled'}>Buy Pack</button></div></article>`}).join('');
  ...
  renderShop=function(){pfV317RenderShopBase();document.querySelectorAll('#shopGrid .shop-mini-pack small').forEach(el=>el.textContent='5 CARDS · 250 CARD SET');const note=document.getElementById('packTierNote');if(note)note.textContent='Core packs are $175. Premium packs are $1,750. Pack odds and value differences are in the ? reference.'};
  ...
  try{if(['3.18','3.19'].includes(PF_VERSION)&&localStorage.getItem(V318_RESET_KEY)!=='1'){clearPackEconomy();clearOldEconomyBackups();localStorage.setItem(V318_RESET_KEY,'1');pfV318DidReset=true;markSaveDirty();save(true);clearOldEconomyBackups()}}catch(e){console.warn('v3.18 economy reset marker unavailable',e)}
  function pfV318ApologyNeeded(){if(!['3.18','3.19'].includes(PF_VERSION))return false;try{return localStorage.getItem(V318_CLAIM_KEY)!=='1'&&!state.v318ApologyClaimed}catch(e){return !state.v318ApologyClaimed}}
  ...
  function pfV320ApologyNeeded(){if(PF_VERSION!=='3.20')return false;try{return localStorage.getItem(V320_APOLOGY_KEY)!=='1'}catch(e){return !state.v320ApologySeen}}
  ...
  /* Persistent custom cards for Admin mode. Custom cards do not expand normal 250-card set completion. */
  ...
  if(!premiumPackDefs.recoverygod321)premiumPackDefs.recoverygod321={id:'recoverygod321',name:'Wyatt Apology God Pack',emoji:'🌟',cost:0,sell:0,adminOnly:true,gradient:'linear-gradient(135deg,#07110b,#1c5a2d,#0b1510)',glow:'#64f095',desc:'Wyatt’s fixed five-card apology pack. Every player receives the exact same cards.',effect:'Jet Lumagui · Mythic #001/025 · Wyatt Evans · Mythic · Holo Serialized Grade 10 BLACK LABEL #001/001 Common'};
  else Object.assign(premiumPackDefs.recoverygod321,{name:'Wyatt Apology God Pack',desc:'Wyatt’s fixed five-card apology pack. Every player receives the exact same cards.',effect:'Jet Lumagui · Mythic #001/025 · Wyatt Evans · Mythic · Holo Serialized Grade 10 BLACK LABEL #001/001 Common'});
  ...
  const pfV321PremiumBase=createPremiumPackPulls;createPremiumPackPulls=function(def,theme,autoMode=false){if(def?.id!=='recoverygod321')return pfV321PremiumBase(def,theme,autoMode);state.packsOpened++;addOverdriveProgress('pack',1);const myth25=makeCopy('Serialized',null,'#001 / 025');myth25.source='Wyatt Apology God Pack · 1/25';myth25.openingQuality='First Print';myth25.premiumTrait='1/25';const one=makeCopy('Holo',null,'#001 / 001');one.grade=10;one.blackLabel=true;one.gradedAt=Date.now();one.source='Wyatt Apology God Pack · 1/1';one.openingQuality='Perfect 1/1';one.premiumTrait='Holo · 1/1';one.gradeNote=generateGraderNote(one,10);const pulls=[pfV321Pull(JET_LUMAGUI_CARD),pfV321Pull(pfV321FixedRarity('medieval','Mythic'),myth25),pfV321Pull(PF_WYATT_EVANS_CARD),pfV321Pull(pfV321FixedRarity('space','Mythic')),pfV321Pull(pfV321FixedRarity('food','Common'),one)];pulls._godPack=true;pulls._godMode='v321_recovery';pulls._packModeDef={tier:'god',name:'Wyatt Apology God Pack',emoji:'🌟',short:'WYATT GOD PACK',desc:'Jet Lumagui · Mythic #001/025 · Wyatt Evans · Mythic · Holo Serialized Grade 10 BLACK LABEL #001/001 Common.'};pulls.sort((a,b)=>pullScore(a)-pullScore(b));state.quirkStats.godPacks=(state.quirkStats.godPacks||0)+1;state.quirkStats.blackLabels=(state.quirkStats.blackLabels||0)+1;markSaveDirty();return pulls};
  ...
  function pfV321ApologyClaimed(){try{return localStorage.getItem(V321_APOLOGY_KEY)==='1'||!!state.v321ApologyClaimed}catch(e){return !!state.v321ApologyClaimed}}
  function pfV321ApologyNeeded(){return !pfV321ApologyClaimed()}
  function pfV321ShowApology(testMode=false){if((!testMode&&!pfV321ApologyNeeded())||document.getElementById('pfV321EconomyNotice'))return;const message=`As you may have seen yesterday, the 15th, there was a glitch where the packs cost way too much, and because they cost a lot, they therefore sold for a lot. This allowed people to click to get these packs, then turn around and sell them for copious amounts of money. You may not have even known about this glitch. Therefore, I am sorry, but I have to reset everybody's money and packs to $0. Blame me. For this, I am so deeply sorry. I will be giving you a free God Pack, one of the best packs in the game. It has the same cards in every single one so as not to be "unfair" to anybody. If you continue to play the game, I thank you and promise nothing like this will ever occur again. If you choose not to play anymore, then, well... that sucks. Also, there was a major update with new things added, so have fun.\n\nSincerely,\nWyatt Evans`;const ov=document.createElement('div');ov.id='pfV321EconomyNotice';ov.className='pf318-apology pf321-apology';ov.innerHTML='<div class="pf318-apology-card"><small>PACKFORGE · A MESSAGE FROM WYATT</small><h1>I owe you another apology.</h1><div class="pf318-type" id="pf321Type"></div><div class="pf318-apology-foot"><span id="pf321Countdown">Please read · 30s</span><button id="pf321Claim" disabled>CLAIM UNLOCKS IN 30s</button></div></div>';document.body.appendChild(ov);const type=document.getElementById('pf321Type'),btn=document.getElementById('pf321Claim'),count=document.getElementById('pf321Countdown'),started=performance.now(),duration=30000;let pos=0,claiming=false;const tick=now=>{const elapsed=now-started,target=Math.min(message.length,Math.floor(message.length*Math.min(1,elapsed/28500)));if(target!==pos){type.textContent=message.slice(0,target);pos=target}const left=Math.max(0,Math.ceil((duration-elapsed)/1000));if(left>0){count.textContent=`Please read · ${left}s`;btn.textContent=`CLAIM UNLOCKS IN ${left}s`;requestAnimationFrame(tick)}else{type.textContent=message;count.textContent='Thank you for reading.';btn.disabled=false;btn.textContent='CLICK TO CLAIM 1 FREE GOD PACK'}};requestAnimationFrame(tick);btn.onclick=()=>{if(btn.disabled||claiming)return;claiming=true;btn.disabled=true;if(!testMode){if(pfV321ApologyClaimed()){ov.remove();toast('Apology pack already claimed','This save can only receive the free apology pack once.');return}state.v321ApologyClaimed=true;try{localStorage.setItem(V321_APOLOGY_KEY,'1')}catch(e){}}grantPremiumPack('recoverygod321',1);markSaveDirty();save(true);renderHUD();renderPacks();updateBagCounts();ov.remove();toast(testMode?'Admin apology test complete':'Wyatt Apology God Pack claimed','+1 fixed Wyatt Apology God Pack in Bag → Packs.')}}
  ...
  window.__pfV322Health=()=>{const seg=window.__pfCasinoBridge?.moneyWheelSegments||[];return({version:PF_VERSION,wheelRTP:seg.length?seg.reduce((a,b)=>a+b,0)/seg.length:0,wheelSegments:[...seg],adjacentBusts:seg.some((v,i)=>v===0&&seg[(i+1)%seg.length]===0),apologyPack:premiumPackDefs.recoverygod321?.effect,apologyCards:[JET_LUMAGUI_CARD.name,pfV321FixedRarity('medieval','Mythic').name+' · Serialized · #001/025',PF_WYATT_EVANS_CARD.name,pfV321FixedRarity('space','Mythic').name,pfV321FixedRarity('food','Common').name+' · Holo · Serialized · Grade 10 · Black Label · #001/001'],wyattDollars:document.querySelectorAll('.pf321-wyatt-fx .cash').length,versionNotes:PF320_PLAYER_LOG});}
  ...
  PF320_PLAYER_LOG.unshift({version:'3.29',title:'Wyatt Apology God Pack',items:['The Wyatt Apology God Pack now contains a guaranteed 1/25 Mythic and a Grade 10 Perfect Black Label 1/1 card.']});
  const pf329Badge=$('#pfVButton');if(pf329Badge){pf329Badge.innerHTML=`<b>V3.29</b><small>NEW</small><span class="pf-vcount-dot">${PF320_PLAYER_LOG.reduce((n,v)=>n+v.items.length,0)}</span>`;pf329Badge.title='New cards, rarities, packs, variants, and features only';pf329Badge.onclick=openVersionLog;pf329Badge.oncontextmenu=e=>{e.preventDefault();pfV320ClaimMatthew()}}
  /* v3.29 repair: if the v3.28 triple-popup bug left multiple un-opened apology packs, keep exactly one. This runs once and never touches cards already opened. */
  const V329_APOLOGY_DEDUPE_KEY='packforge_v329_apology_pack_dedupe';
  let pfV329DuplicatePacksRemoved=0;
  try{if(localStorage.getItem(V329_APOLOGY_DEDUPE_KEY)!=='1'){const n=Math.max(0,Number(state.premiumPackInventory.recoverygod321||0));if(n>1){pfV329DuplicatePacksRemoved=n-1;state.premiumPackInventory.recoverygod321=1;markSaveDirty();save(true)}localStorage.setItem(V329_APOLOGY_DEDUPE_KEY,'1')}}catch(e){console.warn('v3.29 apology pack dedupe marker unavailable',e)}

  window.__pfV329Health=()=>{const setChecks={};for(const id of PF328_SET_IDS){const t=themes.find(x=>x.id===id);setChecks[id]={total:t?.allCards?.length||0,ultra:t?.allCards?.filter(c=>c.rarity==='Ultra').length||0,secret:t?.secretCards?.length||0,ghost:t?.ghostCards?.length||0,god:t?.godCards?.length||0,ascendant:t?.ascendantCards?.length||0,unique:new Set((t?.allCards||[]).map(c=>c.name.toLowerCase())).size,oneWord:(t?.allCards||[]).every(c=>!/\s/.test(c.name))}}const packDef=premiumPackDefs.recoverygod321||{};return{version:PF_VERSION,legacyApologies:{v318:pfV318ApologyNeeded(),v320:pfV320ApologyNeeded()},currentApologyNeeded:pfV321ApologyNeeded(),recoveryPackCount:Number(state.premiumPackInventory.recoverygod321||0),duplicateApologyPacksRemoved:pfV329DuplicatePacksRemoved,recoveryEffect:packDef.effect,sets:setChecks,pokemonAscendants:themes.find(t=>t.id==='pokemon')?.ascendantCards?.map(c=>c.name),moneyWheelPresent:!!document.querySelector('[data-casino-game="moneywheel"],#moneyWheelCard,#moneyWheelSpinBtn'),rewardCodeUIVisible:!!document.querySelector('#pfSettingsPanel-codes:not(.hidden),[data-settings-tab="codes"]'),shop250:[...document.querySelectorAll('#shopGrid .shop-mini-pack small')].every(x=>/250 CARD SET/.test(x.textContent||''))}};
