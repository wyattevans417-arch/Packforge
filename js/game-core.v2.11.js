...
function renderCasino(){try{mccInitCasino()}catch(err){console.error('Meme casino render error',err)}const cash=$('#casinoCashReadout');if(cash)cash.textContent=fmt(state.cash)}
  ...
  /* ==========================================================
     ...
     ========================================================== */
  const PF_UPDATE_ID='presentation-ui-v33-20260913';
  const PF_VERSION='3.3';
  ...
  const PF_V33_ACHIEVEMENTS=[
    ...
    {id:'v33_archive',icon:'∞',name:'Impossible Archive',desc:'Open an Archive God Pack.',cat:'Rare Pulls',tier:'Secret',check:s=>(state.quirkStats?.archiveGodPacks||0)>=1},
  ...
  pfOddsSpecialHTML=function(){
    ...
    else{chanceText=`Shop pack · ${fmt(choice.d.cost)}`;body=choice.d.effect}
    return `<div class="pf-odds-picker-row"><label>Special pack<select id="pfOddsSpecialSelect">${all.map(x=>`<option value="${x.id}" ${x.id===choice.id?'selected':''}>${x.kind==='mutation'?'Mutation':'Shop'} · ${x.name}</option>`).join('')}</select></label></div><div class="pf-odds-feature-detail pack"><div>${pfOddsPackMock(choice.name,choice.emoji,choice.kind==='mutation'?'special':'premium',choice.kind==='mutation'?'SPECIAL MUTATION':'PREMIUM PACK')}</div><div><small>${choice.kind==='mutation'?'NATURAL SPECIAL PACK':'SPECIALTY SHOP PACK'}</small><h3>${pfEscapeHtml(choice.name)}</h3><strong>${chanceText}</strong><p>${pfEscapeHtml(body)}</p></div></div><div class="pf-odds-pack-gallery">${all.map(x=>`<button data-pf-pick-special="${x.id}" class="${x.id===choice.id?'active':''}">${pfOddsPackMock(x.name,x.emoji,x.kind==='mutation'?'special':'premium',x.kind==='mutation'?'MUTATION':'SHOP')}<span>${pfEscapeHtml(x.name)}</span></button>`).join('')}</div>`;
  };
